...
class MANNCell():
    def __init__(self, rnn_size, rnn_layers, memory_size, memory_vector_dim, head_num, gamma=0.95,
    ...
    def read_head_addressing(self, k, prev_M):
        with tf.variable_scope('read_head_addressing'):
            ...
            K_exp = tf.exp(K)
            w = K_exp / tf.reduce_sum(K_exp, axis=1, keepdims=True)  # eq (3)
    ...
    def least_used(self, w_u):  # eq (6)
        _, indices = tf.nn.top_k(w_u, k=self.memory_size)
        b = indices[:, -self.head_num:]
        a = tf.one_hot(b, depth=self.memory_size)
        w_lu = tf.reduce_sum(a, axis=1)
        return indices, w_lu
